const content = document.getElementById("content");
    ...
    function setStatus(message, type) {
      ...
      runStatus.className = "run-status" + (type ? " " + type : "");
      runStatus.scrollIntoView({ behavior: "smooth", block: "nearest" });
    }
    ...
    function updateFinalizeBar(data) {
      ...
      if (!bar) return;
      const onBatchTab = !document.getElementById("batch-section").hidden;
      const show = !onBatchTab && liveSessionId && !sessionFinalized && data?.session?.status !== "ended";
      bar.hidden = !show;
    }
    ...
    function clearSessionForNewInput() {
      ...
      document.getElementById("live-icd").value = "";
      document.getElementById("live-cpt").value = "";
      document.getElementById("live-duration").value = "16";
      document.getElementById("live-session-id").textContent = "";
    ...
    function handleLiveResponse(data) {
      ...
      if (data.session && data.session.session_id) {
        ...
        document.getElementById("live-session-id").textContent = "Session: " + liveSessionId.slice(0, 8) + "…";
      }
    ...
    async function startLiveSession() {
      ...
      try {
        let ruleEl = document.getElementById("live-billing-rule");
        if (!document.getElementById("transcript-section").hidden) {
            ruleEl = document.getElementById("transcript-billing-rule");
        }
        ...
        document.getElementById("live-billing-rule").disabled = true;
        document.getElementById("transcript-billing-rule").disabled = true;
        handleLiveResponse(data);
      } catch (err) {
        setStatus(err.message, "error");
    ...
    async function liveAddIcd() {
      await ensureLiveSession();
      const icd = document.getElementById("live-icd").value.trim();
      if (!icd) { setStatus("Enter an ICD-10 code.", "error"); return; }
      try {
        handleLiveResponse(await liveApi("/live/session/" + liveSessionId + "/icd", "POST", { icd10_code: icd }));
        document.getElementById("live-icd").value = "";
      } catch (err) { setStatus(err.message, "error"); }
    }

    async function liveDetectCpt() {
      await ensureLiveSession();
      const cpt = document.getElementById("live-cpt").value.trim();
      if (!cpt) { setStatus("Enter a CPT code.", "error"); return; }
      try {
        handleLiveResponse(await liveApi("/live/session/" + liveSessionId + "/cpt/detect", "POST", { cpt_code: cpt }));
      } catch (err) { setStatus(err.message, "error"); }
    }
    ...
    function updateSentenceDisplay() {
      if (currentSentenceIndex < parsedSentences.length) {
        document.getElementById("next-sentence-display").textContent = parsedSentences[currentSentenceIndex];
        ...
        document.getElementById("next-sentence-display").textContent = "All sentences fed.";
    ...
    function appendFedSentence(sentenceText) {
      ...
      const div = document.createElement("div");
      div.style.cssText = `margin-bottom: 8px; padding: 10px; background: var(--bg-body); border-left: 4px solid ${speakerHtml ? (speakerHtml.includes('#2563eb') ? '#2563eb' : '#16a34a') : 'var(--border-color)'}; border-radius: 4px;`;
    ...
    async function liveEndCpt() {
      await ensureLiveSession();
      const cpt = document.getElementById("live-cpt").value.trim();
      const duration = parseFloat(document.getElementById("live-duration").value);
      if (!cpt) { setStatus("Enter the CPT code to end.", "error"); return; }
      if (!Number.isFinite(duration) || duration <= 0) {
        setStatus("Enter duration in minutes before ending the CPT.", "error");
        return;
      }
      try {
        handleLiveResponse(await liveApi("/live/session/" + liveSessionId + "/cpt/end", "POST", {
        ...
        if (!document.getElementById("live-cpt").readOnly) {
          document.getElementById("live-cpt").value = "";
        }
      } catch (err) { setStatus(err.message, "error"); }
    }
    ...
    async function liveEndSession() {
      await ensureLiveSession();
      try {
        handleLiveResponse(await liveApi("/live/session/" + liveSessionId + "/end", "POST"));
      } catch (err) { setStatus(err.message, "error"); }
    }

    async function runLiveDemo() {
      await startLiveSession();
      document.getElementById("live-icd").value = "M25.511";
      await liveAddIcd();
      document.getElementById("live-cpt").value = "97110";
      await liveDetectCpt();
      document.getElementById("live-duration").value = "16";
      await liveEndCpt();
      document.getElementById("live-cpt").value = "97140";
      await liveDetectCpt();
      document.getElementById("live-duration").value = "16";
      await liveEndCpt();
      document.getElementById("live-cpt").value = "97530";
      await liveDetectCpt();
      document.getElementById("live-duration").value = "28";
      await liveEndCpt();
    }

    document.getElementById("btn-live-start").addEventListener("click", startLiveSession);
    document.getElementById("btn-live-icd").addEventListener("click", liveAddIcd);
    document.getElementById("btn-live-detect").addEventListener("click", liveDetectCpt);
    document.getElementById("btn-live-end-cpt").addEventListener("click", liveEndCpt);
    document.getElementById("btn-live-end-session").addEventListener("click", liveEndSession);
    ...
    document.getElementById("tab-transcript").addEventListener("click", function () {
      showEditMode();
      document.getElementById("live-section").hidden = true;
      document.getElementById("batch-section").hidden = true;
    ...
    document.getElementById("tab-live").addEventListener("click", function () {
      ...
      document.getElementById("live-section").hidden = false;
      document.getElementById("batch-section").hidden = true;
      document.getElementById("transcript-section").hidden = true;
    ...
    document.getElementById("tab-batch").addEventListener("click", function () {
      showEditMode();
      document.getElementById("live-section").hidden = true;
      document.getElementById("batch-section").hidden = false;
      document.getElementById("transcript-section").hidden = true;
      document.getElementById("finalize-bar").hidden = true;
    ...
    function setBatchStatus(message, type) {
      const el = document.getElementById("run-status-batch");
    ...
    async function evaluatePayload(payload, label) {
      ...
      setBatchStatus("Running evaluation...", "info");
      content.innerHTML = '<div class="loading">Evaluating session...</div>';
      try {
        const session = normalizeSessionPayload(payload);
        const batchRule = document.getElementById("batch-billing-rule");
        if (batchRule && batchRule.value) {
            session.billing_rule = batchRule.value;
        }
        const response = await fetch(apiUrl("/billing/evaluate"), {
          method: "POST",
        ...
        if (!response.ok) {
          throw new Error(formatApiError(errBody.detail || response.statusText, response.status));
        }
        if (!errBody.ui_display) {
          throw new Error("Server returned no ui_display in the report.");
        }
        renderUi(errBody.ui_display);
        setBatchStatus("Evaluation complete.", "success");
        if (label) {
          document.title = "Medexa — " + label;
        }
      } catch (err) {
        setBatchStatus("Evaluation failed.", "error");
        content.innerHTML = '<div class="error">' + escapeHtml(err.message) + '</div>';
      } finally {
    ...
    async function evaluateFixture(fixturePath) {
      const payload = await fetch(apiUrl(fixturePath)).then(r => {
        if (!r.ok) throw new Error(`Failed to load ${fixturePath}`);
        return r.json();
      });
      await evaluatePayload(payload, fixturePath.split("/").pop());
    }
    ...
    document.getElementById("file-input").addEventListener("change", (e) => {
      ...
      reader.onload = () => {
        try {
          document.getElementById("json-paste").value = reader.result;
          const payload = JSON.parse(reader.result);
          evaluatePayload(payload, file.name);
        } catch (err) {
          content.innerHTML = `<div class="error">Invalid JSON in ${escapeHtml(file.name)}: ${escapeHtml(err.message)}</div>`;
        }
      };
      reader.onerror = () => {
        content.innerHTML = `<div class="error">Could not read ${escapeHtml(file.name)}</div>`;
      };
    ...
    function runFromTextarea() {
      ...
      if (!raw) {
        setBatchStatus("Paste session JSON in the box above first.", "error");
        content.innerHTML = '<div class="error">Paste session JSON in the box above first.</div>';
        return;
      }
      try {
        evaluatePayload(JSON.parse(raw), "pasted session");
      } catch (err) {
        setBatchStatus("Invalid JSON.", "error");
        content.innerHTML = '<div class="error">Invalid JSON: ' + escapeHtml(err.message) + '</div>';
      }
    }

    document.getElementById("btn-run").addEventListener("click", runFromTextarea);

    document.getElementById("json-paste").addEventListener("keydown", (e) => {
    ...
    document.getElementById("btn-load-stress").addEventListener("click", () =>
      evaluateFixture("/static/fixtures/stress_test_session.json"));
    document.getElementById("btn-load-capture").addEventListener("click", () =>
      evaluateFixture("/static/fixtures/capture_demo_session.json"));
